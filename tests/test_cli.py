    ...
    def test_cli_with_temperature_settings(self, mock_workflow):
        ...
        with patch.object(sys, 'argv', test_args):
            try:
                ...
                # Note: idea_generation temperature is scaled to min(2.0, base * 1.3)
                temp_manager = kwargs.get('temperature_manager')
                assert temp_manager is not None
                # For base temperature 0.8, idea generation is scaled to min(2.0, 0.8 * 1.3) = 1.04
                assert temp_manager.get_temperature_for_stage('idea_generation') == 1.04
