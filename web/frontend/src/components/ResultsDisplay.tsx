...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ 
  ...
  const handleShare = async (result: IdeaResult, index: number) => {
    ...
    // Set loading state
    setSharingIds(prev => new Set(Array.from(prev).concat(index)));
  ...
  // Helper function to render advocacy content (handles both JSON and text formats)
  const renderAdvocacyContent = (advocacy: any) => {
  ...
  // Helper function to render skepticism content (handles both JSON and text formats)
  const renderSkepticismContent = (skepticism: any) => {
          ...
          return (
            <div className="space-y-4">
              ...
              {parsed.risks_and_challenges && parsed.risks_and_challenges.length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-900 mb-2">RISKS AND CHALLENGES:</h4>
                  <ul className="space-y-2">
                  ...
                  <ul className="space-y-2">
                    {parsed.questionable_assumptions.map((assumption: any, idx: number) => (
                      <li key={idx} className="flex items-start">
                        ...
                        <div>
                          {assumption.assumption && assumption.concern ? (
                            <>
                              <strong>{assumption.assumption}:</strong> {assumption.concern}
                            </>
                          ) : (
                            assumption
                          )}
              ...
              {parsed.missing_considerations && parsed.missing_considerations.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-2">
                    {parsed.missing_considerations.map((missing: any, idx: number) => (
                      <li key={idx} className="flex items-start">
  ...
  const handleExport = async (result: IdeaResult, index: number) => {
    // Set loading state
    setExportingIds(prev => new Set(Array.from(prev).concat(index)));
          ...
          <div key={index} className="p-6 fade-in">
            ...
            <div className="mt-4 flex space-x-3">
              ...
              <button 
                ...
                disabled={sharingIds.has(index)}
                className={`inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md ${
                  sharingIds.has(index) 
                ...
                {sharingIds.has(index) ? 'Sharing...' : 'Share'}
              </button>
              
              <button 
                type="button"
                onClick={() => handleExport(result, index)}
                disabled={exportingIds.has(index)}
                className={`inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md ${
                  exportingIds.has(index) 
                    ? 'text-gray-400 bg-gray-100 cursor-not-allowed' 
                    : 'text-gray-700 bg-white hover:bg-gray-50'
                } focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:hover:bg-gray-100`}>
                {exportingIds.has(index) ? (
                ...
                {exportingIds.has(index) ? 'Exporting...' : 'Export'}
              </button>
            </div>
          </div>
        ))}
